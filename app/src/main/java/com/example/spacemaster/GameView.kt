...
//When the content on background needs to change use SurfaceView
class GameView(activity: GameActivity, screenX: Int, screenY: Int) : SurfaceView(activity), Runnable {
    ...
    private fun update() {
        //when update method called BG will move by 10 pixels and make it compatible with any screen size
        ...
        if (ufo.isGoingUp) //when UFO going up reduce y value by 30
            ufo.y -= (30 * screenRatioY).toInt()
        else
            ufo.y += (30 * screenRatioY).toInt()

        if (ufo.y < 0)
            ufo.y = 0  //prevent fight go off the screen
...
//                }
    ...
    private fun draw() {
        //drawing the BG using canvas on the screen
        if (holder.surface.isValid) {
            val canvas = holder.lockCanvas()  //return the current canvas being displayed on screen
            ...
            //when game over load different UFO image and save score
            if (isGameOver) {
                isPlaying = false
            ...
            holder.unlockCanvasAndPost(canvas) //display the canvas on screen
        }
    }
...
//refreshing the images on screen 60fps (1000/17)
    private fun sleep() {
...
//Resume or start the game
    fun resume() {
        isPlaying = true
        thread = Thread(this)
        thread!!.start() //when thread starts its call run function
    }
...
//Pause the game
    ...
    //make the ufo go up when touch the screen
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> {
                ufo.isGoingUp = false
                if (event.x > screenX / 2) { //if this event is true its mean touch coming from right side of screen
                    ufo.toShoot++
                }
            }
        }
